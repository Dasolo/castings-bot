...
class RawMessage(Base):
    ...
    source_type: Mapped[str] = mapped_column(Text, nullable=False)  # denormalized for fast filtering
    external_msg_id: Mapped[str] = mapped_column(Text, nullable=False)
    raw_text: Mapped[Optional[str]] = mapped_column(Text)
    raw_data: Mapped[Optional[dict]] = mapped_column(JSONB)         # media, metadata
    received_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    parsed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))  # NULL = not parsed yet
...
class SentLog(Base):
    ...
    __table_args__ = (
        # composite PK acts as unique constraint — no duplicates possible
    )
